...
Castillo \i{et al.} uses three different types of content relevance:
...
To compute the authority of the user, Duan \i{et al.} uses four different scores:
\begin{itemize}
...
To test the proposed method the authors of Duan \i{et al.} analyzed hot searches on CrowsEye within a week and collected the messages of 20 of the most frequent searched query terms. The terms included 5 persons 5 persons, 5 locations, 5 products and 5 movie names. 
...
For comparison the authors of Duan \i{et al.} used the Normalized Discount Cumulative Gain (NDCG) metric, which is common to measure the quality of rankings.
...
Castillo \i{et al.} tested different machine learning schemes to run on the data, they included Naive Bayes, Bayes Net, Logistic Regression, and Random Forest. The results of the comparison showed that Bayes Net was the best learning scheme in this scenario, although Random Forest performed almost equally well.

\paragraph{Feature subset}
After the testing of the different learning schemes Castillo \i{et al.} studied how different features  contribute in the prediction of newsworthy topics.
...
With the manually labeled test data Castillo \i{et al.} were able to compare the effects of different feature subsets on the test data. A small part of the results of this comparision is shown in Figure \ref{fig:boxNews}.
...
Apart from the slightly different goals there are also some differences in the approach of the authors of Castillo \i{et al.} and Duan \i{et al.}. Castillo \i{et al.} doesn't consider single tweets as own entities but aggregate many tweets into a unit and study the resulting information cascades. The classifier built by the authors of Castillo \i{et al.} use binary classification (Newsworthy vs Chat or Credible vs Non-Credible).
The smallest unit in Duan \i{et al.} are the text-messages itself, they are not classified in two classes but are rated in a continuously manner for their relevance given a topic.
...
Both paper collect messages from twitter not directly from the news stream but through different tools. Castillo \i{et al.} uses Twitter Monitor whereas Duan \i{et al.} uses query searches form CrowdEye.
