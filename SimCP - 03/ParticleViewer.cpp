...
const int NUM_VERTICES_PER_PARTICLE = 7;
...
int  computeStaticParticleData(SbVec3f *particleNormalBuffer, int *particleFaceIndices)
{
  ...
  for (int mesh = 0; mesh < vectorMeshes.size(); mesh ++)
  {
    ...
    for (int i = 0; i < m.vectorSprings.size(); i ++)
    {
      int *springStart = particleSpringIndices + 3 * spr;
      springStart[0] = part + m.vectorSprings[i].p1 * NUM_VERTICES_PER_PARTICLE + NUM_VERTICES_PER_PARTICLE - 1;
      springStart[1] = part + m.vectorSprings[i].p2 * NUM_VERTICES_PER_PARTICLE + NUM_VERTICES_PER_PARTICLE - 1;
      springStart[2] = SO_END_LINE_INDEX;
      spr ++;
    }

    part += m.vectorParticles.size() * NUM_VERTICES_PER_PARTICLE;
  }
...
void  visualizeParticles()
{
  ...
  {
    Mesh& m = vectorMeshes[mesh];
    for (int i = 0; i < m.vectorParticles.size(); i ++)
    {
      updateParticle(part, m.vectorParticles[i].position.x, m.vectorParticles[i].position.y, m.vectorParticles[i].position.z, coords, m.vectorParticles[i].radius);
      /*for (int j = NUM_FACES_PER_PARTICLE * part; j < NUM_FACES_PER_PARTICLE * part + NUM_FACES_PER_PARTICLE; j ++)
      {
        color[j].setValue(m.vectorParticles[i].r, m.vectorParticles[i].g, m.vectorParticles[i].b);
      }*/
      part ++;
    }
  }

  particleFaces->coordIndex.setValues(0, NUM_FACE_INDICES_PER_PARTICLE * part, particleFaceIndices);
...
int main(int argc, char ** argv )
{
  ...
  particleSpringIndices = new int [3 * MAX_NUM_PARTICLES];
  SbColor * colors = new SbColor [NUM_FACES_PER_PARTICLE * MAX_NUM_PARTICLES];
  int numSprings = computeStaticParticleData(particleNormalBuffer, particleFaceIndices); 

  particleMaterials = new SoMaterial;
  particleMaterials->diffuseColor.setValue(SbColor(1.0, 0.1, 0.1)); //.setValues(0, NUM_FACES_PER_PARTICLE * MAX_NUM_PARTICLES, colors);
  ...
  SoIndexedLineSet *springs = new SoIndexedLineSet;
  springs->coordIndex.setValues(0, 3 * numSprings, particleSpringIndices);
  particleGroup->addChild(springs);
